...
The integration tests verify the completed tasks directly on Solana
devnet:

- SPL mint configuration and supply.
- Successful transfer of one token to the recipient.
- MPL Core NFT ownership, updated name and metadata URI.
- Updated off-chain JSON metadata.

The tests are read-only and do not require a wallet private key or send transactions.
